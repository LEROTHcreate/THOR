...
// ── Tabs ──────────────────────────────────────────────────────────────────────
type Tab = "general" | "equipe" | "agenda" | "tarification" | "donnees";
...
// ══════════════════════════════════════════════════════════════════════════════
function TabGeneral() {
  const [config, setConfig] = useState<StoreConfig>({ nom: "Clair Audition" });
  const [saved, setSaved] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  useEffect(() => { setConfig(loadAuditionStoreConfig()); }, []);
  ...
  function handleSave() {
    saveAuditionStoreConfig(config);
    setSaved(true);
        ...
        <div className="flex items-center gap-5">
          <div className="grid h-16 w-16 flex-shrink-0 place-items-center rounded-2xl overflow-hidden"
            style={config.logo
              ...
              : { background: `linear-gradient(135deg,${PRIMARY},#059669)`, boxShadow: "0 2px 8px rgba(0,201,138,0.25)" }
            }>
            {config.logo ? (
              ...
              <svg className="w-7 h-7 text-white" viewBox="0 0 24 24" fill="none">
                <path d="M12 3c-1.5 0-2.9.4-4.1 1.1L5 2.5A10 10 0 0 0 2 12c0 5.5 4.5 10 10 10s10-4.5 10-10c0-1.9-.5-3.7-1.4-5.2" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
...
// TAB: Équipe
// ══════════════════════════════════════════════════════════════════════════════
const ALL_ROLES: UserRole[] = ["Gérant", "Audioprothésiste", "Optométriste", "Opticien", "Visagiste", "Assistant(e)"];
...
function TabEquipe() {
  ...
  const [form, setForm] = useState<NewUserForm>({ nom: "", prenom: "", email: "", role: "Audioprothésiste", password: "" });
  ...
  function handleAddUser() {
    ...
    setShowModal(false);
    setForm({ nom: "", prenom: "", email: "", role: "Audioprothésiste", password: "" });
  }
...
// TAB: Agenda
// ══════════════════════════════════════════════════════════════════════════════
const GCAL_KEY = "thor_pro_audition_gcal_connected";

function TabAgenda() {
  const [schedule, setSchedule] = useState<WeekSchedule>(DEFAULT_SCHEDULE);
  const [duree, setDuree] = useState<string>("45");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        ...
        // Merge stored days with DEFAULT_DAY_PAUSE for any missing pause fields
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-xl" style={{ background: gcalConnected ? "rgba(0,201,138,0.12)" : "rgba(241,245,249,0.8)" }}>
              <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5" stroke={gcalConnected ? "#00C98A" : "#94a3b8"} strokeWidth="1.8" strokeLinecap="round">
          ...
          <button
            ...
            style={gcalConnected
              ...
              : { background: "rgba(0,201,138,0.10)", color: "#00C98A", border: "1.5px solid rgba(0,201,138,0.25)" }}
          >
...
// TAB: Tarification
// ══════════════════════════════════════════════════════════════════════════════
const TARIF_KEY = "thor_pro_audition_tarification";

interface TarifConfig {
  classeII_adulte:  string;
  classeII_enfant:  string;
  coefMarge:        string;
  remiseMax:        string;
...
const DEFAULT_TARIF: TarifConfig = {
  classeII_adulte: "1490",
  classeII_enfant: "1490",
  coefMarge:       "2.5",
  remiseMax:       "10",
  mentionsDevis:   "Devis valable 30 jours. Prix TTC. Tiers payant selon conditions mutuelle.\nPériode d'essai de 30 jours incluse. Entretien et réglages compris.",
  tauxTVA:         "5.5",
};

function loadTarif(): TarifConfig {
  if (typeof window === "undefined") return DEFAULT_TARIF;
  try {
    const raw = localStorage.getItem(TARIF_KEY);
    return raw ? { ...DEFAULT_TARIF, ...(JSON.parse(raw) as Partial<TarifConfig>) } : DEFAULT_TARIF;
  } catch { return DEFAULT_TARIF; }
}

function saveTarif(t: TarifConfig) {
...
function TabTarification() {
  ...
  function set(k: keyof TarifConfig, v: string) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Prix de vente Classe II */}
      <Card>
        <h2 className="mb-1 text-base font-semibold text-slate-800">Prix de vente par défaut — Classe II</h2>
        <p className="mb-5 text-sm text-slate-500">
          Ces prix sont pré-remplis lors de la création d&apos;un devis Classe II. La Classe I (100% Santé) est plafonnée par la LPPR et n&apos;est pas configurable ici.
        </p>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1.5">Adulte (≥ 20 ans) — € TTC</label>
            <div className="relative">
              <input type="number" min="0" step="10" value={tarif.classeII_adulte}
                onChange={e => set("classeII_adulte", e.target.value)}
          ...
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1.5">Enfant (&lt; 20 ans) — € TTC</label>
            <div className="relative">
              <input type="number" min="0" step="10" value={tarif.classeII_enfant}
                onChange={e => set("classeII_enfant", e.target.value)}
                className="w-full rounded-xl border border-slate-200 bg-white/70 px-3 py-2 text-sm text-slate-800 outline-none pr-8" />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-400">€</span>
            </div>
          </div>
        </div>
        <div className="mt-4 rounded-xl p-3 text-xs text-slate-500 leading-relaxed"
          style={{ background: "rgba(0,201,138,0.05)", border: "1px solid rgba(0,201,138,0.15)" }}>
          <strong className="text-slate-700">Rappel :</strong> la base de remboursement AMO est de <strong>200 € / appareil</strong> (adulte) et <strong>1 400 € / appareil</strong> (enfant). Le RAC patient = prix affiché − AMO − remboursement AMC.
        </div>
      </Card>

      {/* Coefficient & remise */}
      <Card>
        <h2 className="mb-1 text-base font-semibold text-slate-800">Calcul automatique & remises</h2>
        <p className="mb-5 text-sm text-slate-500">
          Utilisé pour calculer un prix de vente suggéré depuis le prix fournisseur, et pour limiter les remises accordées par les praticiens.
        </p>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1.5">Coefficient de marge</label>
            <div className="relative">
              <input type="number" min="1" max="10" step="0.1" value={tarif.coefMarge}
                onChange={e => set("coefMarge", e.target.value)}
                className="w-full rounded-xl border border-slate-200 bg-white/70 px-3 py-2 text-sm text-slate-800 outline-none pr-8" />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-400">×</span>
            </div>
            <p className="text-[11px] text-slate-400 mt-1">Prix fournisseur × coefficient = prix de vente suggéré</p>
          </div>
          <div>
      ...
      {/* TVA & Mentions */}
      <Card>
        <h2 className="mb-1 text-base font-semibold text-slate-800">TVA & mentions devis</h2>
        <p className="mb-5 text-sm text-slate-500">Appliqués automatiquement à tous vos devis et factures Clair Audition.</p>
        <div className="mb-4">
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1.5">Taux TVA (%)</label>
          <div className="flex gap-2">
            {[{ val: "5.5", label: "5,5 % — Taux réduit (aides auditives)" }, { val: "20", label: "20 % — Taux normal" }].map(o => (
              <button key={o.val} onClick={() => set("tauxTVA", o.val)}
                className="flex-1 rounded-xl border px-4 py-2.5 text-sm font-medium transition"
                style={tarif.tauxTVA === o.val
                  ? { borderColor: "#00C98A", background: "rgba(0,201,138,0.08)", color: "#00a574" }
...
// ── Import patients audition ───────────────────────────────────────────────────
const AUD_IMPORT_FIELDS = [
...
const AUD_AUTO_DETECT: Record<string, string[]> = {
...
function audAutoMap(cols: string[]): Record<string, string> {
  const norm = cols.map(c => c.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[\s\-]+/g,"_"));
  const res: Record<string, string> = {};
  for (const [field, patterns] of Object.entries(AUD_AUTO_DETECT)) {
...
function SectionImportAudition() {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setColumns(headers); setRows(bodyRows); setMapping(audAutoMap(headers)); setStep("mapping");
    if (fileRef.current) fileRef.current.value = "";
  }

  function doImport() {
    const existing = readJSONArray<Record<string,unknown>>("thor_pro_audition_patients");
    ...
    localStorage.setItem("thor_pro_audition_patients", JSON.stringify([...existing, ...newPatients]));
    ...
    <Card>
      <div className="flex flex-col items-center gap-4 py-3 text-center">
        <div className="w-12 h-12 rounded-full bg-emerald-50 flex items-center justify-center">
          <svg className="w-6 h-6 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
        </div>
        <div>
  ...
  if (step === "mapping") {
    const visible = AUD_IMPORT_FIELDS.filter(f => mapping[f.key]);
    return (
      <Card>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          {AUD_IMPORT_FIELDS.map(({ key, label, required }) => (
            <div key={key}>
  ...
  return (
    <Card>
      ...
      <button onClick={() => fileRef.current?.click()}
        ...
        style={{ borderColor: "rgba(0,201,138,0.35)", background: "rgba(0,201,138,0.04)", color: PRIMARY }}>
...
function ZoneDangerAudition() {
  ...
  const code = audDailyCode();

  function tryCode() {
    if (inputCode.trim().toUpperCase() === code) {
      ["thor_pro_audition_patients","thor_pro_audition_dossiers","thor_pro_audition_devis",
       "thor_pro_audition_rdv","thor_pro_audition_sav","thor_pro_audition_bilans",AGENDA_KEY]
  ...
  return (
    <Card>
      ...
      <p className="mb-5 text-sm text-slate-500">
        Supprime définitivement toutes les données Audition. Action irréversible — un code de sécurité SAV est requis.
      </p>
      ...
      {step === "idle" && (
        <button onClick={() => setStep("demande")}
          ...
          Réinitialiser les données Audition
        </button>
      )}
...
function TabDonnees() {
  function exportPatients() { downloadCSV("audition_patients.csv", readJSONArray<Record<string,unknown>>("thor_pro_audition_patients")); }
  function exportDossiers() { downloadCSV("audition_dossiers.csv", readJSONArray<Record<string,unknown>>("thor_pro_audition_dossiers")); }
  function exportDevis()    { downloadCSV("audition_devis.csv",    readJSONArray<Record<string,unknown>>("thor_pro_audition_devis")); }
...
// PAGE
// ══════════════════════════════════════════════════════════════════════════════
export default function ParametresAuditionPage() {
  const [activeTab, setActiveTab] = useState<Tab>("general");

  return (
    <div className="mx-auto max-w-3xl pb-16">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-800 h-title">Paramètres</h1>
        <p className="mt-1 text-sm text-slate-500">
          Configuration du cabinet, de l&apos;équipe et des préférences Clair Audition.
        </p>
      </div>
