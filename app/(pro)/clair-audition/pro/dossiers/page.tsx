  ...
  return (
    <Link href={`/clair-audition/pro/dossiers/${d.id}`} style={{ textDecoration: "none", color: "inherit", display: "flex" }}
      ...
      <div className="flex items-center gap-2 min-w-0 w-52 flex-shrink-0">
        <IconHeadphones className="w-4 h-4 text-slate-500 flex-shrink-0" />
        <div className="min-w-0">
      ...
      {/* Date */}
      <div className="text-xs text-slate-500 flex-shrink-0 w-24 hidden lg:block">
        {new Date(d.dateCreation).toLocaleDateString("fr-FR")}
      </div>
      {/* Chevron */}
      <svg className="w-4 h-4 text-slate-500 flex-shrink-0 ml-auto hidden sm:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <div className="text-[10px] font-bold uppercase tracking-widest text-slate-500 self-center mr-1">Marque</div>
        {MARQUES.map(m => (
          <button
            key={m}
      ...
      {/* Status pills */}
      <div className="flex flex-wrap gap-2">
        <div className="text-[10px] font-bold uppercase tracking-widest text-slate-500 self-center mr-1">Statut</div>
        {STATUTS.map(s => {
      ...
      <div className="rounded-2xl overflow-hidden" style={glass}>
        <div className="hidden sm:flex items-center gap-3 px-5 py-3 border-b border-slate-100/60 text-[10px] font-bold uppercase tracking-widest text-slate-500">
        ...
        <div className="divide-y divide-slate-100/60">
          {filtered.length === 0 ? (
            <div className="p-10 text-center text-sm text-slate-500">Aucun dossier ne correspond à vos filtres.</div>
          ) : (
            filtered.map(d => (
