...
/* ── Interfaces ──────────────────────────────────────────────────────────── */
interface Dossier { id:string; patientNom?:string; patientPrenom?:string; status?:string; dateLivraison?:string; type?:string; marque?:string; modele?:string; }
...
/* ── Renewal thresholds ──────────────────────────────────────────────────── */
function renewalThreshold(type?: string) { return type === "lentilles" ? 365 : 730; }
const SOON_OFFSET = 60;
const INACTIVE = 365;

/* ── SMS / Email templates ───────────────────────────────────────────────── */
function smsTemplate(item: RappelItem, centre: string): string {
  if (item.raison === "renouvellement_eligible")
    return `Bonjour ${item.prenom}, votre équipement optique est maintenant éligible au renouvellement pris en charge par la Sécurité Sociale. Contactez ${centre} pour prendre rendez-vous. À bientôt !`;
  if (item.raison === "renouvellement_proche")
    return `Bonjour ${item.prenom}, votre équipement optique sera bientôt éligible au renouvellement SS. Anticipez dès maintenant en prenant rendez-vous chez ${centre}.`;
  return `Bonjour ${item.prenom}, cela fait un moment que nous n'avons pas eu de vos nouvelles. N'hésitez pas à nous contacter chez ${centre} pour un contrôle de vue. À bientôt !`;
}

function emailSubject(item: RappelItem): string {
  if (item.raison === "renouvellement_eligible") return "Votre équipement optique est éligible au renouvellement";
  if (item.raison === "renouvellement_proche")   return "Anticipez le renouvellement de vos équipements optiques";
  return "Prenez soin de votre vue — votre opticien Clair Vision";
}

function emailTemplate(item: RappelItem, centre: string): string {
  if (item.raison === "renouvellement_eligible")
    return `Bonjour ${item.prenom} ${item.nom},\n\nNous vous contactons car votre équipement optique est désormais éligible au renouvellement pris en charge par la Sécurité Sociale.\n\nNous vous invitons à prendre rendez-vous avec notre équipe pour découvrir les nouvelles collections de montures et profiter des meilleures technologies de verres.\n\nCordialement,\nL'équipe ${centre}`;
  if (item.raison === "renouvellement_proche")
    return `Bonjour ${item.prenom} ${item.nom},\n\nVotre équipement optique sera bientôt éligible au renouvellement pris en charge par la Sécurité Sociale.\n\nAnticipez dès maintenant en prenant rendez-vous pour un bilan de vue complet.\n\nCordialement,\nL'équipe ${centre}`;
  return `Bonjour ${item.prenom} ${item.nom},\n\nNous espérons que vous allez bien. Nous n'avons pas eu de vos nouvelles depuis quelque temps et souhaitons vous proposer un contrôle de votre vue.\n\nUn bilan régulier permet de détecter d'éventuelles évolutions de votre correction.\n\nCordialement,\nL'équipe ${centre}`;
}

/* ── Demo data ───────────────────────────────────────────────────────────── */
const DEMO_D: Dossier[] = [
  { id:"d1", patientNom:"Martin",  patientPrenom:"Sophie",  status:"Livré", dateLivraison:"2023-02-10", type:"montures-verres", marque:"Ray-Ban",  modele:"RB5228" },
  { id:"d2", patientNom:"Leblanc", patientPrenom:"Paul",    status:"Livré", dateLivraison:"2022-11-20", type:"montures-verres", marque:"Silhouet", modele:"Titan" },
  { id:"d3", patientNom:"Vidal",   patientPrenom:"Camille", status:"Livré", dateLivraison:"2025-01-05", type:"lentilles",       marque:"Acuvue",   modele:"Oasys" },
];
const DEMO_P: Patient[] = [
  { id:"p1", nom:"Martin",  prenom:"Sophie",  telephone:"06 11 22 33 44", email:"sophie.martin@email.fr",  createdAt:"2022-06-15" },
  { id:"p2", nom:"Leblanc", prenom:"Paul",    telephone:"07 33 44 55 66", email:"paul.leblanc@email.fr",   createdAt:"2022-09-01" },
  { id:"p3", nom:"Vidal",   prenom:"Camille", telephone:"06 55 66 77 88", email:"c.vidal@email.fr",        createdAt:"2023-11-20" },
  { id:"p4", nom:"Dupont",  prenom:"Marc",    telephone:"06 99 88 77 66", email:"m.dupont@email.fr",       createdAt:"2022-03-01" },
];
...
export default function RappelsPage() {
  ...
  useEffect(() => {
    setOk(true);
    setContacted(safeLoad<RappelRecord>("thor_pro_rappels_contacted"));
  }, []);

  useEffect(() => {
    if (modalItem) {
      setSmsText(smsTemplate(modalItem, "Clair Vision"));
      setEmailText(emailTemplate(modalItem, "Clair Vision"));
      setNoteVal("");
    }
  }, [modalItem]);

  const dossiers = useMemo(() => { const d = ok ? safeLoad<Dossier>("thor_pro_dossiers") : []; return d.length ? d : DEMO_D; }, [ok]);
  const patients = useMemo(() => { const p = ok ? safeLoad<Patient>("thor_pro_patients") : []; return p.length ? p : DEMO_P; }, [ok]);
  ...
  const allRappels = useMemo((): RappelItem[] => {
    ...
    dossiers.filter(d => d.status === "Livré" && d.dateLivraison).forEach(d => {
      const threshold = renewalThreshold(d.type);
      ...
      if (days >= threshold) {
        ...
        items.push({ key, nom: d.patientNom ?? "", prenom: d.patientPrenom ?? "",
          ...
          detail: `${d.marque} ${d.modele} — livré le ${fmtDate(d.dateLivraison!)} · ${Math.round((days - threshold) / 30)}m de dépassement`,
          urgence: "haute", days });
      } else if (days >= threshold - SOON_OFFSET) {
        seen.add(key);
        const p = patients.find(p => `${p.nom} ${p.prenom}`.toLowerCase() === `${d.patientNom} ${d.patientPrenom}`.toLowerCase());
        items.push({ key, nom: d.patientNom ?? "", prenom: d.patientPrenom ?? "",
          telephone: p?.telephone, email: p?.email,
          raison: "renouvellement_proche",
          detail: `${d.marque} ${d.modele} — éligible le ${fmtDate(addDays(d.dateLivraison!, threshold))}`,
      ...
      if (days >= INACTIVE) {
        ...
        if (!seen.has(key)) {
          items.push({ key, nom: p.nom ?? "", prenom: p.prenom ?? "",
            ...
            urgence: days > 730 ? "haute" : "normale", days });
        }
      }
    });
  ...
  async function simulateBulkSend(items: RappelItem[], canal: Canal) {
    ...
    items.forEach(item => {
      const msg = canal === "sms" ? smsTemplate(item, "Clair Vision") : emailTemplate(item, "Clair Vision");
    ...
    <div style={{ padding: "28px 32px 52px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 28, display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: "#64748b", margin: 0 }}>
            Envoi de SMS et emails automatiques — renouvellements éligibles et suivi d&apos;activité
          </p>
        </div>
        {selected.size > 0 && (
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button
              onClick={() => simulateBulkSend(selectedItems, "sms")}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 28 }}>
        {[
          ...
          { label: "Bientôt éligibles",           count: aFaire.filter(r => r.raison === "renouvellement_proche").length,   color: A,        bg: `rgba(45,140,255,.08)` },
              ...
              <div key={item.key} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: raColor, background: `${raColor}15`, padding: "2px 10px", borderRadius: 20 }}>
                      {RAISON_LABEL[item.raison]}
                    </span>
                  ...
                  {tab === "aFaire" ? (
                    <>
                      ...
                      <button onClick={() => markContacted(item, "manuel")} style={{
                        padding: "8px 12px", background: "rgba(16,185,129,.1)", color: "#10b981",
                        border: "1px solid rgba(16,185,129,.2)", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer"
                      }}>✓</button>
                    </>
                  ) : (
                    <button onClick={() => unarchive(item.key)} style={{
                      padding: "8px 14px", background: "rgba(100,116,139,.1)", color: "#64748b",
                      border: "1px solid rgba(100,116,139,.2)", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer"
                    }}>
                      Remettre
                    </button>
                  )}
          ...
          <div style={{ background: "#fff", borderRadius: 24, padding: 32, width: "100%", maxWidth: 580, boxShadow: "0 24px 80px rgba(0,0,0,.2)", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: "#64748b", margin: 0 }}>
                  {modalItem.prenom} {modalItem.nom} · {RAISON_LABEL[modalItem.raison]}
                </p>
              </div>
            ...
            {modalItem.telephone && (
              <div style={{ marginBottom: 20 }}>
                ...
                />
                <button
                  onClick={() => simulateSend(modalItem, "sms", smsText)}
                  disabled={!!sending}
            ...
            {modalItem.email && (
              <div style={{ marginBottom: 20 }}>
                ...
                <textarea
                  ...
                  rows={6}
                  style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: `1px solid ${A}25`, fontSize: 13, color: "#334155", outline: "none", resize: "vertical", fontFamily: "inherit", lineHeight: 1.6, boxSizing: "border-box", background: `rgba(45,140,255,.03)` }}
                />
                <button
                  onClick={() => simulateSend(modalItem, "email", emailText)}
                  disabled={!!sending}
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "space-between", flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={() => markContacted(modalItem, "manuel", undefined, noteVal || undefined)} style={{ padding: "10px 18px", background: "rgba(16,185,129,.1)", color: "#10b981", border: "1px solid rgba(16,185,129,.2)", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  ✓ Marquer contacté
                </button>
              </div>
            </div>
