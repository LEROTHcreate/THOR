...
function IconEye() {
  return <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none"><path d="M2.5 12s3.5-7 9.5-7 9.5 7 9.5 7-3.5 7-9.5 7-9.5-7-9.5-7Z" stroke="currentColor" strokeWidth="1.7"/><circle cx="12" cy="12" r="3" stroke="currentColor" strokeWidth="1.7"/></svg>;
}
function IconLens() {
  return <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="1.7"/><circle cx="12" cy="12" r="4" stroke="currentColor" strokeWidth="1.7"/></svg>;
}
function IconFile() {
...
const NAV = [
  ...
  { href: "/clair-vision/espace-patient/mon-profil",     label: "Mon profil",     Icon: IconUser    },
  { href: "/clair-vision/espace-patient/examens-de-vue", label: "Examens de vue", Icon: IconEye     },
  { href: "/clair-vision/espace-patient/lentilles",      label: "Lentilles",      Icon: IconLens    },
  { href: "/clair-vision/espace-patient/ordonnances",    label: "Ordonnances",    Icon: IconFile    },
  { href: "/clair-vision/espace-patient/documents",      label: "Documents",      Icon: IconFile    },
  { href: "/clair-vision/espace-patient/achats",         label: "Achats",         Icon: IconCart    },
  { href: "/clair-vision/espace-patient/messages",       label: "Messages",       Icon: IconMessage },
  { href: "/clair-vision/espace-patient/rendez-vous",      label: "Rendez-vous",       Icon: IconCalendar        },
  { href: "/clair-vision/espace-patient/pre-consultation", label: "Pré-consultation", Icon: IconClipboardCheck  },
  { href: "/clair-vision/espace-patient/centres",          label: "Mes centres",      Icon: IconMap             },
  { href: "/clair-vision/espace-patient/actualites",       label: "Actualités",       Icon: IconBell            },
];
    ...
    <>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV.map(({ href, label, Icon }) => {
          const active = pathname === href || (href !== "/clair-vision/espace-patient" && pathname.startsWith(href));
          return (
...
export default function VisionPatientLayout({ children }: { children: ReactNode }) {
    ...
    <div className="relative flex h-screen"
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around px-2 py-2"
        ...
        {[
          ...
          { href: "/clair-vision/espace-patient/ordonnances",   label: "Ordonn.",   icon: <IconFile /> },
          { href: "/clair-vision/espace-patient/messages",      label: "Messages",  icon: <IconMessage /> },
          { href: "/clair-vision/espace-patient/mon-profil",    label: "Profil",    icon: <IconUser /> },
        ].map((item) => {
          const active = pathname === item.href || (item.href !== "/clair-vision/espace-patient" && pathname.startsWith(item.href));
          return (
            <Link key={item.href} href={item.href}
              className="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all min-w-[52px]"
              style={active ? { color: "#2D8CFF" } : { color: "#94a3b8" }}>
              <span className={`flex items-center justify-center w-7 h-7 rounded-xl transition-all ${active ? "bg-[rgba(45,140,255,0.12)]" : ""}`}>
                {item.icon}
              </span>
      ...
      {/* ── Assistant IA ── */}
      <ChatWidget context="patient-vision" />
    </div>
  );
}
